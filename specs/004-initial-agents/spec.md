...
When new People are created during world generation, they should have realistic and varied `firstName` and `lastName` values generated by the system.

**Why this priority**: Name realism improves immersion but is secondary to correct initialization and UI access.

**Independent Test**: Run world generation (with deterministic and non-deterministic seeds) and validate `firstName`/`lastName` values for People created during generation according to uniqueness and pattern metrics (see Success Criteria).
...
**Agent**: Represents a person assignment record that references a `Person` entity from the canonical models. Key attributes: unique ID, reference to `Person` (source of `firstName`/`lastName`, `homeZoneId`, `skills`, `attributes`), role/title. Note: People currently have no age field and are treated as adults.
...
- **SC-005**: Name generator uniqueness: in a sample of 1,000 generated `firstName`+`lastName` pairs, ≥95% are unique and ≥99% match a sane name pattern (letters, spaces, typical punctuation).
