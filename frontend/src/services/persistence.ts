...
export async function listConfigs(): Promise<Array<{ name: string; updatedAt: number }>> {
  const db = await getDB()
...
export async function importConfig(json: string, nameOverride?: string): Promise<string> {
